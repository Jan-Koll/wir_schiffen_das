...
func worker(ctx context.Context, jobChan <-chan *job) {
	for {
		...
		case job, ok := <-jobChan:
			...
			processJob(job)
			triggerJob("cooling_exhaust_analyzer", os.Getenv("COOLING_EXHAUST_ANALYZER_PORT"), job.ConfigurationId, job.OrderCreatedAt)
			triggerJob("mounting_analyzer", os.Getenv("MOUNTING_ANALYZER_PORT"), job.ConfigurationId, job.OrderCreatedAt)
			triggerJob("propulsion_analyzer", os.Getenv("PROPULSION_ANALYZER_PORT"), job.ConfigurationId, job.OrderCreatedAt)
			triggerJob("supply_analyzer", os.Getenv("SUPPLY_ANALYZER_PORT"), job.ConfigurationId, job.OrderCreatedAt)
		}
	}
}
...
func triggerJob(hostname string, port string, configuration_id int, order_created_at time.Time) (err error) {
	target_url := fmt.Sprintf("http://%s:%s/v1/job", hostname, port)
	data, err := json.Marshal(jobTrigger{ConfigurationID: configuration_id, OrderCreatedAt: order_created_at})
	if err != nil {
