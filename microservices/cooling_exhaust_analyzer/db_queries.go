...
func getAnalysisById(id int) (result job, err error) {
	...
	err = row.Scan(&result.JobId, &result.ConfigurationId, &result.Success, &result.CoolingSystem, &result.ExhaustSystem, &result.CreatedAt, &result.LastModified, &result.Status, &result.OrderCreatedAt)
	return
}
...
func createJob(input job) (result job, err error) {
	result = input
	row := db.QueryRow(
		`INSERT INTO cooling_exhaust_analyzer (configuration_id, status, cooling_system, exhaust_system, order_created_at)
		 VALUES ($1, $2, $3, $4, $5)
...
func getAnalysesByConfigurationId(configuration_id int) (result []job, err error) {
	...
	for rows.Next() {
		var job job
		err = rows.Scan(&job.JobId, &job.ConfigurationId, &job.Success, &job.CoolingSystem, &job.ExhaustSystem, &job.CreatedAt, &job.LastModified, &job.Status, &job.OrderCreatedAt)
		if err != nil {
